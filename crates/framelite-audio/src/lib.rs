//! # framelite-audio — the audio sink adapter
//!
//! The audio sink pulls [`AudioBlock`]s from the **data plane** ([`framelite_media`]), not the
//! serializing JSON bus: audio is high-bandwidth and must never be encoded. It rides a
//! [`bounded`](framelite_media::bounded), **lossless** channel, so a slow consumer applies
...
use framelite_media::{AudioBlock, BoundedReceiver};

...
use micro_bus::{LocalBus, Overflow};
use micro_core::{JoinReport, Module, Runtime};
use micro_document::Doc;
use micro_protocol::Channel;
...
/// # fn build_source() -> impl micro_core::Module { Src }
/// # fn build_sink() -> impl micro_core::Module { Src }
/// # struct Src;
/// # impl micro_core::Module for Src {
/// #   fn id(&self) -> micro_core::ModuleId { micro_core::ModuleId::new("x") }
/// #   fn run(self: Box<Self>, _: micro_core::ModuleCtx) {}
/// # }
/// let mut app = App::new();
...
impl App {
    ...
    /// liveness for a status overlay; pair with [`bus().metrics()`](micro_bus::LocalBus::metrics).

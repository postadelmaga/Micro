//! # framelite-input — the input source
//!
//! Input is the **source** end of framelite's `sources → world → sinks` dataflow. A real UI
...
//! framework means framelite never pins a windowing stack or its version, an app can target
...
//! # use framelite_protocol::Topic;
//! # use framelite_input::{InputEvent, InputMapper, Key};
...
use framelite_bus::{LocalBus, ModuleId};
use framelite_protocol::Topic;
use serde::Serialize;
...
mod tests {
    ...
    fn mapper_decode(rx: &dyn framelite_bus::Receiver) -> Option<Move> {

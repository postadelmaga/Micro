//! # micro-core — the module runtime
//!
//! This crate is the micro-kernel: it composes [`micro_bus`] and [`micro_document`]
...
//!                     let _ = bus.publish(
//!                         micro_protocol::Envelope::new(me, "result", answer),
//!                     );
//!                 });
//!             }
//!             Ok(None) => {}
...
//! }
//! # }
//! # fn expensive(_e: &micro_protocol::Envelope) -> serde_json::Value { serde_json::Value::Null }
//! ```
...
use micro_bus::{LocalBus, Receiver};
use serde::Serialize;
pub use micro_bus::{BusError, LocalBus as Bus};
pub use micro_protocol::{Channel, Envelope, ModuleId, Topic};
...
impl Runtime {
    ...
    /// shutdown or panic) — pair it with [`LocalBus::metrics`](micro_bus::LocalBus::metrics)

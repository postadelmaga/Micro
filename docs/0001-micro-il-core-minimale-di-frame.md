# ADR 0001 — Micro: il core minimale e generico di Frame

- **Stato**: Accettato
- **Data**: 2026-06-28
- **Decisori**: Fra
- **Relazione con Frame**: Micro **estrae e generalizza** il cuore architetturale di Frame
...
Creare `Micro` come **workspace sorella** di `Frame`, che implementa SOLO il core e lo rende
**generico**:

1. **Canali a stringa** invece dell'enum fisso di Frame. In Frame `ChannelId` è
   `Audio|Scene|Control|Input` (dominio ZenFlow); in Micro un canale è un `Channel(String)`,
...
2. **Un solo transport: `LocalBus` in-process.** Frame ha Local + Ipc + ring shmem per i sidecar;
   Micro tiene solo il broker pub/sub in-memory. I sidecar/rete restano una *cucitura* (i
...
3. **`Doc<S>` generico undoable**, senza migrazioni né schemi di dominio. Ogni edit è un
   ...
   comando ha successo).
4. **Niente dominio.** Nessun renderer, audio, scene, plugin. L'app che usa Micro li aggiunge.

## 3. Architettura

```
micro-protocol   ModuleId · Channel(String) · Envelope · ChannelKind     (zero logica)
        ▲
micro-bus        trait Sender/Receiver + LocalBus (pub/sub, retained channels)
micro-document   History<S> · Command<S> · Doc<S>                          (undo/redo)
        ▲                        ▲
micro-core       Module · ModuleCtx · Runtime          (micro-kernel in-process)
```
...
In Micro la distinzione non è cablata nel nome del canale: l'app marca i canali stateful con
...
**Costi:** Micro **non** è eseguibile come app (niente UI/host) — è una libreria-fondamenta;
